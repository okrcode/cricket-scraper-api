...
def fetch_live_match(event_id, max_retries=3, retry_delay=2):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not isinstance(data, dict):
                logger.warning(f"Invalid JSON structure for event {event_id}")
                continue
            ...
            logger.error(f"Timeout error for event {event_id} (attempt {attempt + 1}): {e}")
            if attempt == max_retries - 1:
                return None
        except requests.exceptions.ConnectionError as e:
            logger.error(f"Connection error for event {event_id} (attempt {attempt + 1}): {e}")
            if attempt == max_retries - 1:
                return None
        except requests.exceptions.RequestException as e:
            logger.error(f"Request error for event {event_id} (attempt {attempt + 1}): {e}")
            if attempt == max_retries - 1:
                return None
        except json.JSONDecodeError as e:
            logger.error(f"JSON decode error for event {event_id} (attempt {attempt + 1}): {e}")
            if attempt == max_retries - 1:
                return None
        except Exception as e:
            logger.error(f"Unexpected error for event {event_id} (attempt {attempt + 1}): {e}")
            if attempt == max_retries - 1:
                return None
